...
data_subset=data_all_raw_nor[,which(group_list%in%c("CT","MCI","PD","AD"))]
group_list=group_list[group_list%in%c("CT","MCI","PD","AD")]


# 2.search degs 
contrast_class=c("PD-MCI","PD-CT","CT-MCI","PD-AD","CT-AD","AD-MCI")
...
deg_pd_ad=data_degs[[4]]
deg_ad_ct=data_degs[[5]]
deg_ad_mci=data_degs[[6]]
...
deg4_cutoff=deg4[deg4$adj.P.Val<fdr_cutoff&deg4$logFC>fc_cutoff,]
deg5_cutoff=deg5[deg5$adj.P.Val<fdr_cutoff&deg5$logFC>fc_cutoff,]
deg6_cutoff=deg6[deg6$adj.P.Val<fdr_cutoff&deg6$logFC>fc_cutoff,]
union_gene_three=unique(c(rownames(deg1_cutoff),rownames(deg2_cutoff),rownames(deg3_cutoff),
                          rownames(deg4_cutoff),rownames(deg5_cutoff),rownames(deg6_cutoff))
                        )
...
# 4.write the table 
write.table(union_gene_three,file="R_data/ct_pd_mci_ad_uniondegs.txt",row.names=TRUE,col.names=TRUE,sep="\t")
write.table(data_subset,file="R_data/ct_pd_mci_ad_exp.txt",row.names=TRUE,col.names=TRUE,sep="\t")
write.table(group_list,file="R_data/ct_pd_mci_ad_label.txt",row.names=FALSE,col.names=TRUE,sep="\t")
