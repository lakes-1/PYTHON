...
load("R_data/step02_intersect_degs.Rdata")
...
plotDEG_dataType1=function(data_name,label,loc){
  ...
  data1=apply(data[rownames(brainVerseBlood),], 1,function(x){(x-min(x))/{max(x)-min(x)}})
  group_list=gsub("0","CT",group_list)
  group_list=gsub("1","PD",group_list)
  data2=as.data.frame(cbind(data1,group_list))
  return(data2)
}
...
data_62283=data_62283[deg_62283_loc,]
data1=apply(data_62283, 1,function(x){(x-min(x))/{max(x)-min(x)}})
...
save(blood_protein_62283,blood_rna_99039,brain_protein_68719,brainSN_rna_20292,brainSn_rna_7621,
     brain_rna_68719,file="R_data/03_degs.Rdata")
